...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //Cek apakah $uploadOksudah di-set menjadi 0 oleh error
    if ($uploadOk == 0) {
        echo "<div class='alert alert-danger'>File tidak berhasil diunggah.</div>";
    } else {
        if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
            //Jika file berhasil diunggah, tambahkan ke database
            $sql= "INSERT INTO alumni (nama, tahun_lulus, jurusan, foto) VALUES ('$nama', '$tahun_lulus', '$jurusan', '$target_file')";
            if ($conn->query($sql) === TRUE) {
                echo "<div class='alert alert-danger'>Data berhasil ditambahkan.</div>";
            } else {
                echo "<div class='alert alert-danger'>Error: ".$sql."<br>".$conn->error."</div>";
            }
        } else {
            echo "<div class='alert alert-danger'>Terjadi kesalahan saat mengguganggah file.</div>";
        }
    }
} else {
